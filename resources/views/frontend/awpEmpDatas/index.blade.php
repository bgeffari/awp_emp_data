        ...
        <div class="col-md-12">
            @can('awp_emp_data_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.awp-emp-datas.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-AwpEmpData">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $awpEmpData->id }}">
                                        ...
                                        <td>
                                            @can('awp_emp_data_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.awp-emp-datas.show', $awpEmpData->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('awp_emp_data_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.awp-emp-datas.edit', $awpEmpData->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('awp_emp_data_delete')
                                                <form action="{{ route('frontend.awp-emp-datas.destroy', $awpEmpData->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('awp_emp_data_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.awp-emp-datas.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-AwpEmpData:not(.ajaxTable)').DataTable({ buttons: dtButtons })
