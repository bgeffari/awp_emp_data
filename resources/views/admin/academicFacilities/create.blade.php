...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.academicFacility.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.academic-facilities.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="academic_facility">{{ trans('cruds.academicFacility.fields.academic_facility') }}</label>
                <input class="form-control {{ $errors->has('academic_facility') ? 'is-invalid' : '' }}" type="text" name="academic_facility" id="academic_facility" value="{{ old('academic_facility', '') }}" required>
