...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.awp-emp-datas.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="emp_sap_number">{{ trans('cruds.awpEmpData.fields.emp_sap_number') }}</label>
                <input class="form-control {{ $errors->has('emp_sap_number') ? 'is-invalid' : '' }}" type="number" name="emp_sap_number" id="emp_sap_number" value="{{ old('emp_sap_number', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nid') ? 'is-invalid' : '' }}" type="number" name="nid" id="nid" value="{{ old('nid', '') }}" step="1" required>
                @if($errors->has('nid'))
                    <div class="invalid-feedback">
                        {{ $errors->first('nid') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\AwpEmpData::LAST_CERTIFICATE_COUNTRY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('last_certificate_country') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="last_certificate_country_{{ $key }}" name="last_certificate_country" value="{{ $key }}" {{ old('last_certificate_country', '') === (string) $key ? 'checked' : '' }} required>
