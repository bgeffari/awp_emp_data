...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.subCertificateType.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sub-certificate-types.update", [$subCertificateType->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="sub_certificate_type">{{ trans('cruds.subCertificateType.fields.sub_certificate_type') }}</label>
                <input class="form-control {{ $errors->has('sub_certificate_type') ? 'is-invalid' : '' }}" type="text" name="sub_certificate_type" id="sub_certificate_type" value="{{ old('sub_certificate_type', $subCertificateType->sub_certificate_type) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('main_certificate_types') ? 'is-invalid' : '' }}" name="main_certificate_types[]" id="main_certificate_types" multiple required>
                    @foreach($main_certificate_types as $id => $main_certificate_type)
                        <option value="{{ $id }}" {{ (in_array($id, old('main_certificate_types', [])) || $subCertificateType->main_certificate_types->contains($id)) ? 'selected' : '' }}>{{ $main_certificate_type }}</option>
                    @endforeach
                </select>
